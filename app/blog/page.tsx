      ...
      <div className="pt-36 pb-20 bg-brand-dark min-h-screen">
        <section className="container mx-auto px-6 mb-16">
          <div className="max-w-3xl">
            <div className="inline-block px-3 py-1 mb-4 border border-brand-primary/30 rounded-full bg-brand-primary/5 text-brand-primary text-xs font-sans uppercase tracking-widest">Insights</div>
            <h1 className="font-display text-5xl font-bold text-brand-text mb-4 tracking-tight">Ecommerce Growth Blog.</h1>
            ...
            <div className="flex flex-wrap gap-2">
              {Array.from(new Set(posts.map((p) => p.category))).map((cat) => (
                <Link key={cat} href={`/blog/category/${cat.toLowerCase().replace(/\s+/g, '-')}`}
                  className="px-4 py-2 bg-brand-text/[0.03] border border-brand-border rounded-full text-sm text-brand-muted hover:text-brand-primary hover:border-brand-primary/30 transition-all">
                  {cat}
                </Link>
              ))}
            </div>
